...
rk_randomseed.argtypes = [ct.c_void_p]

rk_seed.restype = None
rk_seed.argtypes = [ct.c_long, ct.c_void_p]

rk_interval.restype = ct.c_ulong
rk_interval.argtypes = [ct.c_ulong, ct.c_void_p]

state = rk_state()
state_p = ct.pointer(state)
state_vp = ct.cast(state_p, ct.c_void_p)
state_value = state_vp.value

def seed(N):
    return rk_seed(N, state_value)

# Returns a random unsigned long between 0 and max inclusive
def interval(max):
    return rk_interval(max, state_value)

def init():
    if rk_randomseed(state_value) != 0:
        raise ValueError("Cannot initialize the random number generator.")

def init2(n=200):
    if rk_seed(n, state_value) != 0:
        raise ValueError("Cannot initialize the random number generator.")
